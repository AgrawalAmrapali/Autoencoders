"""  Classes and Function for creating Autoencoder with detector. Autoencoder with detector basic function."""
## Agrawal Amrapali
##
## 05:25 pm, Friday April 07, 2023 (IST)
##
...
class Autoencoder(object):
    ...
    def __init__(self, n_layers, transfer_function=tf.nn.softplus, optimizer=tf.train.AdamOptimizer(), ae_para=[0, 0]):
        ...
        print("type",type(self.reconstruction))
        # self.reconstruction = np.clip(self.reconstruction, 0.0, 1.0)
        # tf.clip_by_value(self.reconstruction, 0.0, 1.0, name=None)



        # cost
        if self.sparse_reg == 0:
            diff = tf.abs(self.reconstruction - self.x)
            print(tf.subtract(self.reconstruction, self.x))
